...
def test_execute_python_to_csv_allows_stdlib_import(tmp_path):
    tool = ExecutePythonToCsvTool(output_dir=str(tmp_path))
    request = ToolRequest(
        tool_name="execute_python_to_csv",
        inputs={
            "code": "import random\nrows=[{'id': i, 'v': random.randint(1, 9)} for i in range(1,4)]",
            "file_name": "import_ok.csv",
        },
    )
    result = tool.run(request)
    assert result.success is True
    assert (tmp_path / "import_ok.csv").exists()


def test_execute_python_to_csv_blocks_unknown_import(tmp_path):
    tool = ExecutePythonToCsvTool(output_dir=str(tmp_path))
    request = ToolRequest(
        tool_name="execute_python_to_csv",
        inputs={
            "code": "import faker\nrows=[{'id':1}]",
            "file_name": "import_block.csv",
        },
    )
    result = tool.run(request)
    assert result.success is False
    assert "import not allowed" in result.message
